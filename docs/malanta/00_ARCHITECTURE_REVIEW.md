...
הסקירה הזו היא **הניתוח**. כל הממצאים בה יושמו — הסכמה נבנתה מחדש
לפי הדרישות ולא להפך.

| קובץ | תוכן |
|---|---|
| `db/migrations/0001..0010` | הסכמה המלאה, 48 טבלאות |
| `db/seed/0001` | קטלוג התפקידים וההרשאות |
| `db/down/` | rollback לכל מיגרציה |
| `db/tests/rls_matrix.sql` | 49 טענות הרשאות |
| `db/README.md` | סדר הרצה ומלכודות |
| `60_PERMISSION_MODEL.md` | מודל ההרשאות, העריכה, הפרטיות והזכויות |
| `30_PAYMENTS.md` | מודל המסחר המלא |

**מה שהשתנה מעבר לתיקוני הסקירה** (לפי הדרישה "בלי פשרות"):
- הרשאות עברו ממודל תפקידים למודל **הרשאות כנתונים** — `roles`/`permissions`/
  `role_permissions`, עם היקף מפות פר-עובד
- נוספה שכבת **עריכה**: `lock_version` (נעילה אופטימית) ו-`content_revisions`
- נוספה שכבת **פרטיות**: הסכמות בגרסאות, בקשות נושא מידע, אנונימיזציה
- נוספה שכבת **זכויות יוצרים**: מקור, טביעת אצבע, canary, סימני מים, תביעות
- **מסחר מלא**: קטלוג, מחירים, קופונים, הזמנות, זיכויים, חשבוניות, מנויים
- `grant` מפורש לכל טבלה — ברירת המחדל היא "נעול", לא "פתוח"

## מה נבדק בפועל

הכל הורץ מול PostgreSQL 16 אמיתי.

- ✅ 10 מיגרציות + seed נטענות נקי
- ✅ **up → down → up** מלא: 48 טבלאות → 0 → 48, ואז הבדיקות עוברות שוב
- ✅ **49 טענות הרשאות עוברות**, כולל בידוד יוצרים, תפקידים, זכאות מטייל,
  פקיעה, גישת אדמין בהסכמה, 7 ניסיונות הסלמה, ואי-הסגרת קיום בשגיאות

### באגים שהתגלו בהרצה ותוקנו
| # | מה | איך התגלה |
|---|---|---|
| 1 | `PRIMARY KEY`/`UNIQUE` עם ביטוי — לא SQL תקין | טעינת `0007` |
| 2 | `app.is_reserved_slug` בלי `grant execute` → **כל כתיבה ל-`maps`/`tenants` נכשלת** | חלק 8 בבדיקות |
| 3 | ה-down מוחק פונקציות לפני הטריגרים שתלויים בהן | הרצת ה-down |
| 4 | `t_denied` בלע את הודעות הכישלון של עצמו — 4 בדיקות דיווחו הצלחה שגויה | קריאת הפלט |
| 5 | **UPDATE שנחסם ב-RLS לא זורק שגיאה, מעדכן 0 שורות** | תיקון #4 חשף אותו |

באג 2 היה שובר-ייצור. באג 5 הוא מלכודת שיטתית: בדיקה שמצפה לשגיאה על
UPDATE תעבור תמיד, גם כשהמדיניות פרוצה לחלוטין. לכן `t_no_rows`.

### מה עדיין לא נבדק
דורש PostGIS, כלומר פרויקט Supabase אמיתי: העמודה המחושבת `geom`,
אינדקס GIST, `gin_trgm_ops`, ו-`extensions.digest` לטביעת האצבע.
ה-CI ב-`.github/workflows/malanta-db.yml` רץ על `postgis/postgis:16-3.4`
ויכסה גם אותם.
