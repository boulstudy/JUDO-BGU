...
const INIT_DRILLS = [
  ...
  { id:8, name:"שושין",            duration:180, type:"group",   note:"מדיטציה וסיכום" },
];
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.88)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
      <div style={{background:"#0d1020",border:"1px solid rgba(255,107,0,0.35)",borderRadius:18,width:"100%",maxWidth:700,maxHeight:"90vh",overflowY:"auto",padding:28,direction:"rtl"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
          <h2 style={{color:"#FF6B00",fontFamily:"Heebo,sans-serif",fontSize:22,margin:0}}>מערך האימון</h2>
            ...
            {editId === d.id ? (
              <div style={{display:"flex",flexDirection:"column",gap:9}}>
                <div style={{display:"flex",gap:7,flexWrap:"wrap"}}>
                  ...
                  {ed.type === "partner" && (
                    <select value={ed.activeColor||"both"} onChange={e => setEd({...ed,activeColor:e.target.value})} style={inp}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:10}}>
                ...
                <div style={{display:"flex",gap:5}}>
                  <button onClick={() => { const a=[...list]; const t=i-1; if(t>=0){[a[i],a[t]]=[a[t],a[i]]; setList(a); }}} style={{background:"none",border:"1px solid rgba(255,255,255,0.12)",color:"#fff",borderRadius:6,width:28,height:28,cursor:"pointer",fontSize:13}}>up</button>
                  <button onClick={() => { const a=[...list]; const t=i+1; if(t<a.length){[a[i],a[t]]=[a[t],a[i]]; setList(a); }}} style={{background:"none",border:"1px solid rgba(255,255,255,0.12)",color:"#fff",borderRadius:6,width:28,height:28,cursor:"pointer",fontSize:13}}>dn</button>
        ...
        <div style={{borderTop:"1px solid rgba(255,255,255,0.08)",paddingTop:16,marginTop:8}}>
          <p style={{color:"rgba(255,107,0,0.7)",fontSize:13,marginBottom:9,fontFamily:"Heebo,sans-serif"}}>+ תרגיל חדש</p>
          <div style={{display:"flex",gap:7,flexWrap:"wrap"}}>
            <input value={nd.name} onChange={e => setNd({...nd,name:e.target.value})} placeholder="שם" style={{...inp,flex:1,minWidth:100}} />
            <input type="number" value={nd.duration} onChange={e => setNd({...nd,duration:e.target.value})} style={{...inp,width:70}} />
            <select value={nd.type} onChange={e => setNd({...nd,type:e.target.value})} style={inp}>
              <option value="group">קבוצה</option>
              <option value="partner">זוגות</option>
              <option value="personal">אישי</option>
            </select>
            {nd.type === "partner" && (
              <select value={nd.activeColor||"both"} onChange={e => setNd({...nd,activeColor:e.target.value})} style={inp}>
                <option value="both">שניהם</option>
                <option value="white">לבן</option>
                <option value="blue">כחול</option>
              </select>
            )}
            <input value={nd.note} onChange={e => setNd({...nd,note:e.target.value})} placeholder="הערות" style={{...inp,flex:1,minWidth:80}} />
            <button onClick={() => { if(!nd.name.trim()) return; setList([...list,{...nd,id:Date.now(),duration:parseInt(nd.duration)||60}]); setNd({name:"",duration:60,type:"group",note:"",activeColor:"both"}); }} style={{background:"#FF6B00",border:"none",color:"#fff",borderRadius:9,padding:"8px 20px",cursor:"pointer",fontFamily:"Heebo,sans-serif",fontWeight:700,fontSize:15}}>הוסף</button>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.88)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
      <div style={{background:"#0d1020",border:"1px solid rgba(255,107,0,0.35)",borderRadius:18,width:"100%",maxWidth:580,maxHeight:"90vh",overflowY:"auto",padding:28,direction:"rtl"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
          <h2 style={{color:"#FF6B00",fontFamily:"Heebo,sans-serif",fontSize:22,margin:0}}>גודוקות</h2>
        ...
        {list.map(j => (
          <div key={j.id} style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:10,marginBottom:8,overflow:"hidden"}}>
            <div style={{display:"flex",alignItems:"center",gap:10,padding:"12px 16px"}}>
              ...
              <button onClick={() => setExpandedId(expandedId===j.id ? null : j.id)} style={{background:"rgba(255,255,255,0.06)",border:"none",color:"rgba(255,255,255,0.6)",borderRadius:7,padding:"5px 12px",cursor:"pointer",fontSize:13,fontFamily:"Heebo,sans-serif"}}>{expandedId===j.id ? "סגור" : "תרגילים"}</button>
              <button onClick={() => setList(list.filter(x => x.id !== j.id))} style={{background:"none",border:"none",color:"rgba(255,60,60,0.6)",cursor:"pointer",fontSize:18}}>X</button>
            </div>
            {expandedId === j.id && (
              <div style={{borderTop:"1px solid rgba(255,255,255,0.06)",padding:"12px 16px",background:"rgba(0,0,0,0.2)"}}>
                {(j.personalDrills||[]).map((pd,pi) => (
                  <div key={pd.id} style={{display:"flex",alignItems:"center",gap:8,marginBottom:7}}>
                    ...
                    <button onClick={() => setList(list.map(x => x.id===j.id ? {...x,personalDrills:x.personalDrills.filter((_,k) => k!==pi)} : x))} style={{background:"none",border:"none",color:"rgba(255,60,60,0.5)",cursor:"pointer",fontSize:15}}>X</button>
                  </div>
                ))}
        ...
        <div style={{borderTop:"1px solid rgba(255,255,255,0.08)",paddingTop:16,marginTop:8,display:"flex",gap:8,alignItems:"center"}}>
          <input value={newName} onChange={e => setNewName(e.target.value)} placeholder="שם גודוקה חדש" style={{...inp,flex:1}} />
          <select value={newColor} onChange={e => setNewColor(e.target.value)} style={inp}>
            <option value="white">לבן</option>
            <option value="blue">כחול</option>
          </select>
...
export default function JudoTV() {
  ...
  const activeColor = (current && current.activeColor) || "both";

  useEffect(() => {
    if (isPersonal) {
      const init = {};
      judokas.forEach(j => {
        init[j.id] = { drillIdx: 0, timeLeft: (j.personalDrills && j.personalDrills[0] && j.personalDrills[0].duration) || 0 };
      });
  ...
  const pct = timeLeft / ((current && current.duration) || 1);
  ...
  const progress = ((doneTime + ((current ? current.duration : 0) - timeLeft)) / totalDur) * 100;
  ...
  return (
    <div style={{width:"100vw",height:"100vh",overflow:"hidden",background:"#080a10",display:"flex",flexDirection:"column",direction:"rtl",fontFamily:"Heebo,sans-serif",position:"relative"}}>
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"14px 40px",borderBottom:"1px solid rgba(255,255,255,0.06)",background:"rgba(0,0,0,0.3)",flexShrink:0,position:"relative",zIndex:10}}>
        <div style={{display:"flex",alignItems:"center",gap:16}}>
          ...
          <div>
            ...
            <div style={{color:"rgba(255,107,0,0.65)",fontSize:13,letterSpacing:4,textTransform:"uppercase"}}>Ben-Gurion University</div>
          </div>
        </div>
        ...
        <div style={{display:"flex",gap:10}}>
          {[["מערך","drills"],["גודוקות","judokas"],["זוגות","pairs"]].map(([label,key]) => (
      ...
      <div style={{flex:1,display:"flex",overflow:"hidden",padding:"20px 32px",gap:24,position:"relative",zIndex:1}}>
        <div style={{width:260,display:"flex",flexDirection:"column",gap:0,overflowY:"auto",flexShrink:0}}>
            ...
            {drills.map((d,i) => {
              ...
              return (
                <div key={d.id} onClick={() => goTo(i)} style={{background:curr?"rgba(255,107,0,0.14)":done?"rgba(255,255,255,0.015)":"rgba(255,255,255,0.04)",border:curr?"1px solid rgba(255,107,0,0.5)":"1px solid rgba(255,255,255,0.06)",borderRadius:10,padding:"11px 14px",cursor:"pointer",display:"flex",alignItems:"center",gap:10,opacity:done?0.38:1,transition:"all 0.2s"}}>
                  <span style={{width:26,height:26,borderRadius:"50%",background:curr?"#FF6B00":done?"rgba(255,255,255,0.07)":"rgba(255,255,255,0.05)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,color:curr?"#fff":"rgba(255,255,255,0.3)",flexShrink:0}}>{done ? "V" : i+1}</span>
          ...
          <div style={{marginTop:"auto",paddingTop:16,borderTop:"1px solid rgba(255,255,255,0.06)",display:"flex",flexDirection:"column",gap:8}}>
            {[["זמן שעבר",fmt(totalElapsed)],["סהכ אימון",fmt(totalDur)]].map(([l,v]) => (
        ...
        <div style={{flex:1,display:"flex",flexDirection:"column",gap:16,minWidth:0}}>
          <div style={{display:"flex",alignItems:"baseline",gap:16}}>
            <h1 style={{color:"#fff",fontSize:"clamp(28px,4vw,52px)",fontWeight:900,letterSpacing:-1,lineHeight:1}}>{current ? current.name : ""}</h1>
            {current && <span style={{padding:"5px 16px",borderRadius:20,background:TYPE_COLOR[current.type]+"20",color:TYPE_COLOR[current.type],fontSize:16,fontWeight:700,flexShrink:0}}>{TYPE_LABEL[current.type]}</span>}
            {current && current.note && <span style={{color:"rgba(255,255,255,0.35)",fontSize:18}}>{current.note}</span>}
          </div>
          ...
          <div style={{display:"flex",gap:12,justifyContent:"center"}}>
            ...
            <button onClick={() => setTimeLeft(current ? current.duration : 0)} style={{background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.1)",color:"#fff",borderRadius:12,padding:"16px 28px",cursor:"pointer",fontFamily:"Heebo,sans-serif",fontWeight:700,fontSize:20}}>אפס</button>
          ...
          {!isPersonal ? (
            <div style={{background:"rgba(255,255,255,0.02)",border:"1px solid rgba(255,255,255,0.07)",borderRadius:16,padding:"18px 20px",flex:1,display:"flex",flexDirection:"column"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
                ...
                <span style={{color:current?TYPE_COLOR[current.type]:"#fff",fontSize:13,fontWeight:700}}>{isPartner?(activeColor==="white"?"לבן עובד":activeColor==="blue"?"כחול עובד":"שניהם עובדים"):"כולם"}</span>
              </div>
                  ...
                  return (
                    <div key={pi} style={{display:"flex",gap:8,alignItems:"stretch",height:90}}>
                      <div style={{flex:1,borderRadius:12,background:whiteActive?"linear-gradient(145deg,#d0d0d0 0%,#ffffff 100%)":"rgba(255,255,255,0.04)",border:whiteActive?"2px solid rgba(255,255,255,0.9)":"2px solid rgba(255,255,255,0.07)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:4,transition:"all 0.5s ease",boxShadow:whiteActive?"0 0 30px rgba(255,255,255,0.18)":"none"}}>
                        ...
                        <span style={{color:whiteActive?"#111":"rgba(255,255,255,0.22)",fontWeight:800,fontSize:17,transition:"color 0.5s"}}>{w ? w.name : "?"}</span>
                      ...
                      <div style={{flex:1,borderRadius:12,background:blueActive?"linear-gradient(145deg,#003b8e 0%,#1a5fd6 100%)":"rgba(255,255,255,0.04)",border:blueActive?"2px solid #1a5fd6":"2px solid rgba(255,255,255,0.07)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:4,transition:"all 0.5s ease",boxShadow:blueActive?"0 0 30px rgba(26,95,214,0.35)":"none"}}>
                        <span style={{fontSize:22}}>🥋</span>
                        <span style={{color:blueActive?"#fff":"rgba(255,255,255,0.22)",fontWeight:800,fontSize:17,transition:"color 0.5s"}}>{b ? b.name : "?"}</span>
            ...
            <div style={{background:"rgba(255,255,255,0.02)",border:"1px solid rgba(160,255,120,0.2)",borderRadius:16,padding:"18px 20px",flex:1,display:"flex",flexDirection:"column"}}>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:8,flex:1,justifyContent:"center"}}>
                {judokas.map(j => {
                  ...
                  const drillIdx = pt ? pt.drillIdx : 0;
                  const drill = j.personalDrills && j.personalDrills[drillIdx];
                  const tl = pt ? pt.timeLeft : (drill ? drill.duration : 0);
                  const total = drill ? drill.duration : 1;
                  const urgP = (tl / total) < 0.25;
                  return (
                    <div key={j.id} style={{borderRadius:12,padding:"12px 16px",background:j.color==="white"?"linear-gradient(135deg,rgba(255,255,255,0.1),rgba(255,255,255,0.04))":"linear-gradient(135deg,rgba(26,95,214,0.2),rgba(0,59,142,0.1))",border:j.color==="white"?"1px solid rgba(255,255,255,0.25)":"1px solid rgba(26,95,214,0.45)",display:"flex",alignItems:"center",gap:12}}>
                      ...
                      <div style={{flex:1,minWidth:0}}>
                        ...
                        <div style={{color:"rgba(255,255,255,0.4)",fontSize:13,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{drill ? drill.name : "סיים"}</div>
                      </div>
      ...
      {modal === "drills" && <DrillEditorModal drills={drills} setDrills={d => { setDrills(d); if(idx >= d.length){ const ni=d.length-1; setIdx(ni); setTimeLeft(d[ni] ? d[ni].duration : 0); } else { setTimeLeft(d[idx] ? d[idx].duration : 0); } }} currentIndex={idx} onClose={() => setModal(null)} />}
